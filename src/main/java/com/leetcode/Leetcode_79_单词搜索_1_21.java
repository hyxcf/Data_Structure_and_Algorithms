...
public class Leetcode_79_单词搜索_1_21 {
    ...
    private static class Preview_7_13 {
        ...
        public boolean exist(char[][] board, String word) {
            ...
            for (int i = 0; i < row; i++) {
                for (int j = 0; j < col; j++) {
                    if (backtack(0, i, j, board, visited, word)) {
                        return true;
                    }
                }
            }
        ...
        private boolean backtack(int index, int i, int j, char[][] board, boolean[][] visited, String word) {
            if (index == word.length()) {
                return true;
            }
            if (i < 0 || j < 0 || i >= board.length || j >= board[0].length || visited[i][j] || word.charAt(index) != board[i][j]) {
            ...
            visited[i][j] = true;
            if (backtack(index + 1, i + 1, j, board, visited, word)
                    || backtack(index + 1, i - 1, j, board, visited, word)
                    || backtack(index + 1, i, j + 1, board, visited, word)
                    || backtack(index + 1, i, j - 1, board, visited, word)
            ) {
                return true;
            } else {
