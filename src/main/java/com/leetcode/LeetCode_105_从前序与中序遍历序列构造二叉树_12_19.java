...
public class LeetCode_105_从前序与中序遍历序列构造二叉树_12_19 {
    ...
    private static class Preview_4_11 {
        ...
        public TreeNode buildTree(int[] preorder, int[] inorder) {
            ...
            return dfs(0, 0, inorder.length - 1, map, preorder);
        }

        private TreeNode dfs(int preIndex, int left, int right, Map<Integer, Integer> map, int[] preorder) {
            if (left > right) {
                return null;
            }
            TreeNode node = new TreeNode(preorder[preIndex]);
            Integer index = map.get(preorder[preIndex]);
            node.left = dfs(preIndex + 1, left, index - 1, map, preorder);
            node.right = dfs(preIndex + 1 + index - left, index + 1, right, map, preorder);
            return node;
        }
    }
}
